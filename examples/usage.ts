...
async function main() {
  ...
  // Query ETH balance
  const ethBalance = await stateQueryService.getEthBalance(
  ...
  console.log(
    `Transferring ${transferAmount.toString()} tokens to ${recipient.getValue()}`
  );

  try {
    const receipt = await walletService.transfer(recipient, transferAmount);
  ...
  try {
    const reconstructedToken =
      await stateQueryService.reconstructStateFromEvents(tokenAddress);

    console.log(
      "Reconstructed Total Supply:",
      reconstructedToken.getTotalSupply().toString()
    );
    console.log(
      "Reconstructed Balance:",
      reconstructedToken.getBalance(walletService.getAddress()).toString()
    );
